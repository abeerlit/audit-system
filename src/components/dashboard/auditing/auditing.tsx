...
const Auditing = () => {
  ...
  }, [auditingData, selectedChapters.chapterNames_id, searchTest]);
            ...
            <tbody>
              {filteredAuditingData?.map((product: any) => (
                <tr key={product.id} className="hover:bg-gray-100 border-t">
                  ...
                  <td className="p-4 text-nowrap flex items-center">
                    <div className={`rounded-[40px] text-[12px] font-bold text-white px-[10px] py-[4px] ${product.itemAction === 'accept'
                        ? 'bg-green-500'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols xl:grid-cols-3 2xl:grid-cols-4 xl:gap-8 md:gap-6 gap-4">
          {filteredAuditingData.map((product: any) => (
            <div key={product.id} className="relative p-4 bg-white rounded-3xl">
              ...
              />
              <p
                className={`absolute top-[30px] left-[30px] rounded-[40px] text-[12px] font-bold text-white px-[10px] py-[4px] ${product.itemAction === 'accept'
                    ? 'bg-green-500'
                    : product.itemAction === 'skip'
                      ? 'bg-gray-500'
                      : product.itemAction === 'edit'
